...
public class ProceduralLigthFlicekr : MonoBehaviour
{
    // Varibaler
    private int[] perm = new int[512]; // opsætter et overflow int array, for vores hashtabel senere i koden.
    float offsetX, SEED, maxintencity;

    // Unity elementer
    private Light spotlight;
    public Slider SeedField;
    public Slider flickerspeedslider;
    public Slider maxintencityslider;
    public TMP_Text SeedText;
    

    private void Start() {
        initPerlinNoise(); // Opstiller vores hashtable, til værdier for vores vektor grid table.
        spotlight = GetComponent<Light>(); // Henter vores Ligth component på objectet vores script sidder på.
    }

    private void Update() {
        offsetX = offsetX + flickerspeedslider.value; // tilføjer en værdi til vores offset,
                                                      // for at flytte vores perlin grid over tid.
                                                      // Det sørger for at vores lysstyrke ændre sig over tid. 
                                                      // offsettet kan også ses som flicker hastigthed.
        
        maxintencity = maxintencityslider.value; //opdatere vores max lysstyrke
        SEED = SeedField.value;                 // opdatere vores SEED
        SeedText.text = System.Convert.ToString(SeedField.value);   //opdatere vores SEED UI element.
        
        flickerlight(); //Kalder vores function.                                     
    }

        
    public void flickerlight(){
       float xcord = SEED + offsetX;    // Beregner vores punkt vi vil beregne vores værdi af.
       float noisevalue = Pnoise(xcord , 1, 1); //Finder vores værdi af punktet beregnet i sidste linje, ved hjælp af pelin noise.
       spotlight.intensity = noisevalue * maxintencity; // vi tildeler vores spolight itensitet værdieren af vores noise* lysstyrke.
    }
